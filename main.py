...
def generate_meal_plan(age, gender, weight):
    return {
        "plans": [{
            ...
            "meals": [
                {"text": "Crêpes à l’avoine et fromage blanc"},
                {"text": "Poulet tikka light, riz basmati, concombre (portion légère)"},
                {"text": "Poulet, semoule, ratatouille (portion légère)"}
            ],
            "snacks": [{"text": "Smoothie lait + fruit + avoine"}]
        }],
        "shopping_list": {
            "flocons d’avoine": {"qty": 70, "unit": "g"},
...
if __name__ == "__main__":
    ...
    print(f"🚀 NutriAI API sur http://0.0.0.0:{port}")
    httpd.serve_forever()